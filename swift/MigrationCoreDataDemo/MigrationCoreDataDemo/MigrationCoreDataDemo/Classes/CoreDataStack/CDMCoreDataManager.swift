...
class CDMCoreDataManager: NSObject
{
    // MARK: - Singleton
    
    class var sharedManager :CDMCoreDataManager
    {
        struct Singleton
        {
            static let instance = CDMCoreDataManager()
        }
    ...
    lazy var mom: NSManagedObjectModel =
    {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("MigrationCoreDataDemo", withExtension: "momd")!
    ...
    lazy var psc: NSPersistentStoreCoordinator? =
    {
        ...
        let dbName = "MigrationCoreDataDemo.sqlite"
        let url = CDMUtils.sharedInstance.applicationDocumentsDirectory.URLByAppendingPathComponent(dbName)
